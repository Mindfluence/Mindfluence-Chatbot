...
import type { EnhancedEntity } from '@/types/nlp.types';
import type { Entity } from '@/types/nlp.types';
import type { NLPModel } from '@/types/nlp.types';
...
export async function getFaqResponse(
  ...
  try {
    ...
    if (isKnownIntent(intentName) && language in INTENT_DATA[intentName].search) {
      ...
      if (mappedResults && mappedResults.length > 0 && mappedResults[0]?.question) {
      ...
      if (entityTerms.length > 0) {
        ...
        if (enhancedResults && enhancedResults.length > 0 && enhancedResults[0]?.question) {
    ...
    if (faqResults && faqResults.length > 0 && faqResults[0]?.question) {
    ...
    console.log(`[DEBUG] Verwende kontextbezogene Fallback-Antwort: "${fallbackResponse.substring(0, 40)}..."`);
...
function getRelevantEntityTerms(entities: EnhancedEntity[]): string {
  const keywordEntities = entities.filter(entity => 
    entity.type === 'keyword' || 
    entity.type === 'topic' || 
    entity.type === 'product' ||
    entity.type === 'service' ||
  ...
  return sortedEntities.slice(0, 3).map(e => e.value).join(' ');
}
...
function generateMappedFallbackResponse(intentName: KnownIntentName, language: Language): string {
  // Hole den sprachspezifischen Anzeigenamen aus dem Mapping
  const displayName = INTENT_DATA[intentName].display[language];
...
function buildOptimizedSearchQuery(
  ...
  // 1. Beginne mit dem bereinigten Intent-Namen
...
function generateContextAwareFallback(
  ...
  if (isKnownIntent(intentName) && language in INTENT_DATA[intentName].display) {
    const displayName = INTENT_DATA[intentName].display[language];
    return language === 'de'
      ? `Entschuldigung, ich habe keine spezifischen Informationen zu "${displayName}". Möchtest du deine Frage umformulieren oder nach etwas anderem fragen?`
      : `I'm sorry, I don't have specific information about "${displayName}". Would you like to rephrase your question or ask about something else?`;
  ...
  if (intentName.startsWith('faq_')) {
    const topic = intentName.replace(/^faq_/i, '').replace(/_/g, ' ');
  ...
  const topEntity = entities.find(e => e.isNew) || entities[0];
  if (topEntity && topEntity.value) {
    return language === 'de'
      ? `Ich habe leider keine genauen Informationen zu "${topEntity.value}" gefunden. Kann ich dir mit etwas anderem helfen?`
...
export function optimizeModel(trainedModel: NLPModel, validationResult: any): import("@/types/nlp.types").NLPModel | PromiseLike<import("@/types/nlp.types").NLPModel> {
